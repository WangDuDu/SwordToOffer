...
public class NumberOf1Between1AndN {
    ...
    public int numberOf1Between1AndN_Solution (int n) {
        int result  = 0;
        int x = 1;
        int high = 1, temp, current, low;
        int i = 1; //i表示从右到左的第几位

        while (high != 0) {
            high = n / (int)Math.pow(10, i); //当前位的高位
            temp = n % (int)Math.pow(10, i);
            current = temp / (int)Math.pow(10, i-1); //当前位的数字
            low = temp % (int)Math.pow(10, i-1); //当前位的低位

            if (current > x) {
                result += high * (int)Math.pow(10, i-1) + (int)Math.pow(10, i-1);
            } else if (current == x) {
